...
derivativeMonomial v (c, xs) | v == fst (head xs) = (c * snd (head xs),  (fst (head xs), snd (head xs) - 1):tail xs)
                             | otherwise = (fst d, head xs : snd d) where d = derivativeMonomial v (c, tail xs) -- var didn't match
